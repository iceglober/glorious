...
reviewer the preamble while dropping the verdict. When a round fails exactly as the one before it did, character for character, the reviewer is told
so — an edit that claims success and changes nothing looks like progress from inside a single round,
and a reviewer shown only one-line summaries of earlier rounds has nothing to compare against. Only
the newest round is sent in full: earlier
...
Before planning, the runner samples a `Workspace` — cwd, entry names two levels deep (less `.git`,
`node_modules`, and the event log, which sits in the working directory by default and is the agent's
own history rather than part of the project), budgeted eight children to a directory so one crowded
folder cannot crowd out the rest, and, inside a repository, the git root, the
