...
- the generated Extension API referenced `Shipped`, `ExtensionChoice`, and its `WriteOutcome` dependency without exporting them from its entry point. all are now exported, removing TypeDoc's unresolved-reference warnings.
...
8. **shipped extension roster** — export the private bundled roster and generate name/default/summary tables plus the model's availability copy.
