# glrs — component survey at `b4e11c9`


Surveyed at `cc4b0e9` and updated after the nine fix PRs that followed (#307 and #309–#315). Twenty-four of the forty-five items that were not on a live path are resolved; twenty-one remain, three of them deliberately.
...
- `--version` prints `glrs <version>` and returns without touching the terminal, config or git. It wins wherever it sits in argv; it used to require being the only argument. `argv.ts:route` — [CLI]
...
- `--model provider/id` writes `process.env.GLRS_MODEL` before anything reads a model, so it applies to the TUI and `-p` alike. Read by cmd-ts through a type that refuses another flag as its value and refuses an id naming no provider; a missing value is an error rather than silence. `argv.ts:ModelId`, `argv.ts:route` — [CLI, gated `--model`]
- `--resume <id>` opens that session or fails `Session not found`; bare `--resume` opens the picker. The value is typed, so `--resume --model x` is the picker plus a model rather than a session called `--model`. `argv.ts:SessionId`, `argv.ts:route` — [TUI, gated `--resume`]
...
- Unrecognised `--name value` pairs are lifted out before parsing and carried to the extensions that may claim them; cmd-ts rejects anything else. Capitals are carried too — the old scan was lowercase-only, so `--Foo` vanished without even the `(unknown flag:)` line. `argv.ts:liftExtensionFlags` — [CLI + ext API; the `flag.run` half is gated on an extension calling `g.flag()`]
- `--help` prints usage, and is the one route that loads extensions — a subcommand an extension added is discoverable only by asking it, so `glrs wt` is listed from what the extension registered rather than from a fixed list. `argv.ts:route`, `argv.ts:helpText`, `index.ts:main` — [CLI]
...
- Everything after `-p` is the prompt, taken verbatim — it is free text, so it does not pass through the parser and may contain what looks like a flag. Whichever of `-p` and a bare word comes first wins, so `glrs wt -p hi` is the worktree subcommand and `glrs -p what failed` is a prompt rather than a subcommand called `failed`. `argv.ts:route` — [-p]
...
- Model resolution order: `GLRS_MODEL`/`GLORIOUS_MODEL`, then `config.model`. **There is no third place.** Nothing configured throws `NoModelChosen` naming the three ways to set one, and a model naming no provider is refused rather than assumed to be azure — the former default was also the one provider whose base URL was dropped, so the guess and the misconfiguration compounded. `models.ts:currentModel`, `models.ts:modelRef` — [config]
...
- Full transcript replay on resume before the screen starts: every stored event is rendered into scrollback and consecutive tool calls get their run footer exactly as they did live. Extensions have loaded by replay time, so a tool renderer or markdown transform an extension registered applies to history as well as to live output. The replay still precedes the startup notices, so the transcript reads first. It is printed once into scrollback rather than re-rendered on later paints, which is why the order matters for the whole session. `index.ts:762`, `index.ts:763`, `index.ts:767`, `index.ts:773`, `render.ts:378`, `render.ts:319` — [TUI]
...
- `/fork [n]` copies this session to a new id — the whole thing, or its first `n` events — and prints the `glrs --resume <id>` that opens it. The original is untouched. Written in the builtins extension with no core API of its own: `g.session()` names the session and an extension may reach `glrs-core`. `builtins/src/index.ts`, `session.ts:forkSession` — [TUI, gated nothing]
...
- Cache breakpoints are written into the messages for the providers that need them. OpenAI and Google cache a prefix unasked; Anthropic gets `cacheControl`, Bedrock gets `cachePoint`, and a Claude model on Vertex is marked the Anthropic way because the mark follows the model rather than the host. The mark sits on the second-to-last message — the newest point still present next turn — and advances each turn, which these providers' prefix matching turns into an extending cache rather than a fresh one. `shaping.ts:withCacheBreakpoints`, `shaping.ts:cacheHint`, `agent.ts` — [internal]
...
- Skill roots, in order: `~/.config/agents/skills`, then `.agents`, `.glrs` and `.glorious` under `$HOME`, then all three under every ancestor from the root up to `$HOME`, then all three at the project root, then each loaded extension's `skills/` directory — deduped, first root to claim a name wins. `skills.ts:AGENT_DIRECTORIES`, `skills.ts:skillRoots` — [TUI, -p]
...
- `allowed-tools` in a skill's frontmatter restricts the turn that activated it, in the TUI and under `-p`. `activate_skill` is always kept, so a narrow list cannot trap the model inside the skill it just loaded; in `-p` the restriction composes with configured bans rather than replacing them. The list splits on commas as well as whitespace. `skills.ts:OnActivate`, `index.ts:holdToSkill`, `print.ts:holdToSkill` — [TUI, -p]
...
- The root package exports `.` → `sdk.ts` and `./extension-api` → `public-extension-api.ts`, so `import { createCodingAgent } from "@glrs-dev/glrs"` resolves. It declared no `main`, `module` or `exports` at all, while the manifest naming `sdk.ts` as an entry is `private: true` — nothing could reach it, though it shipped in every tarball and TypeDoc built the docs site from it. `package.json:exports` — [build]
- `Glrs` is declared once, in `glrs-core`, and the agent implements it. It was two declarations that had drifted 26 members against 44, so eighteen members worked at runtime and were invisible to anyone writing an extension. The declaration lives in `glrs-core` because `packages/extensions` may not import the coding agent — which is what forced the copy in the first place — and an implementation that falls behind now fails to compile. `glrs-core/src/index.ts`, `extension-api.ts:createApi` — [ext API]
- `Tone` and `Span` had drifted the same way: seven tones painted against five declared, and `italic`/`underline` honoured by the renderer and absent from the type. Both are declared once. `glrs-core/src/index.ts` — [ext API]
- `SkillSummary` was declared twice too, which is how `license` and `metadata` came to be parsed in one and absent from the other. `glrs-core/src/index.ts` — [ext API]
...
What the survey could not settle from source. Three entries are gone: `--frozen-lockfile` was
verified working, the SDK entry now has an `exports` map, and the per-provider option shapes were
read out of the installed SDK type definitions rather than reasoned.
...
The 21 items that remain, gathered. Twenty-four more were resolved by the fix PRs and are
no longer listed above.

| Component | Items |
|---|---|
| 1. CLI entry and dispatch | `index.ts:194` |
| 2. Hosts | `cli.ts:46` |
| 3. Configuration | `config.ts:205` |
| 4. Models and providers | `createProviderRegistry`; `ModelOption.apiKey`; `providers.ts:139`; `providers.ts:143`; `models.ts:232` |
| 7. Agent turn loop | `QUEUE_MODES`; `agent.ts:237` |
| 8. Prompt and context assembly | `mentions.ts:212`; `loadAgentRules` |
| 10. Tools and tool plumbing | `toolkit.ts:105` |
| 12. Bundled extensions | `repoName` |
| 13. Terminal UI | `transcript`; `render.ts:339` |
| 14. Public API surface (SDK and extension types) | `packages/glrs-core/src/index.ts` (163 lines) — live as a type declaration, dead as a module; `createAgentCore`; `Extension` |
| 15. Build, packaging and release | `packages/glrs-coding-agent/bin/glorious`; `.changeset/pre.json:5` |

**Three are kept deliberately.** `providers.ts:139` and `:143` are unreachable today and both
defensive — one guards an empty `env` producing an empty `join`, the other distinguishes an
env-var need from a config-path need; deleting them makes the code worse. `transcript`
(`render.ts`) is test-only rather than unused, and **two** suites import it, so removing it means
duplicating the fold into both.
