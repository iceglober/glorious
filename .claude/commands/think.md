...
- Update the task's items via `gs-agentic state task update --id <id> --items '<json>'`
- Update acceptance criteria via `gs-agentic state task update --id <id> --acceptance '<criteria>'`
- Set the design field via `gs-agentic state task update --id <id> --design '<summary>'`
