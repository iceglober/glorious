...
- `glorious-core` — provider-neutral domain contracts and extension SDK.
- `provider-registry` — provider adapters, credentials, and metadata.
- `glorious-coding-agent` — terminal coding-agent composition boundary.

These internal packages are private implementation boundaries. The root
`@glrs-dev/glorious` package is the only npm distribution.
