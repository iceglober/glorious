...
export const createScreen = async (callbacks: {
  ...
  // Taking over the composer area is the whole of glrs's input primitive.
  ...
  const syncShellMode = (): void => {
    ...
    input.placeholder = shellMode ? "Run shell command" : "Ask Glrs anything";
    draw();
  };
  ...
  const stop = (): void => {
    ...
    renderer.destroy();
    process.stdout.write(`\u001b[2J\u001b[HContinue with: glrs --resume ${callbacks.sessionId}\n`);
  };
